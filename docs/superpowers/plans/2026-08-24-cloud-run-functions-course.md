# Cloud Run Functions Course Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Publish Developing Applications with Cloud Run Functions on Google Cloud as the sixth ACE single-page course.

**Architecture:** Add a second page to the existing Cloud Run course-series directory and reuse the current MkDocs presentation. Normalize headings only outside fenced code blocks so shell comments and code samples remain source content.
...
- Create: `tests/fixtures/ACE_Developing_Applications_with_Cloud_Run_Functions_on_Google_Cloud.source.txt`

**Interfaces:**
- Consumes: the 933-line UTF-8 supplied source.
...
**Files:**
- Create: `docs/courses/ace/developing-applications-with-cloud-run/developing-applications-with-cloud-run-functions-on-google-cloud.md`
- Modify: `mkdocs.yml`
...
- Produces: one Cloud Run Functions page reachable from all discovery surfaces after Fundamentals.

- [ ] Transform only headings outside fenced code and preserve every code comment.
- [ ] Add the Functions child entry, sixth homepage rail/card, and sixth learning-path course.
...
- [ ] Commit with `feat: add Cloud Run Functions course notes` and push `main`.
